...
### 16. [合併專案畫面測試 (Merge UI Test)](./merge-ui-test/SKILL.md)
**「工具回報已點擊，不等於頁面有反應。」** 合併專案畫面測試的瀏覽器操作層工具箱，補足《合併專案畫面測試流程計畫書》沒寫的「怎麼驅動瀏覽器」；判定準則仍以計畫書為準，本技能不複述。

- **操作 HOW 與判定 WHAT 分離**：計畫書管測什麼、怎麼判定，本技能管怎麼點、怎麼落證據、點不到時怎麼辦。
- **元件手冊以實測為據**：Select 的 placeholder `pointer-events: none`、選項需帶 `[aria-selected]` 過濾、虛擬滾動截斷、下拉 portal 到 body、antd 兩字按鈕插空白（「提 交」）——每條標記 `[實測]` 或 `[通例]`，未驗證的不冒充事實。
- **五級降級順序**：第 4、5 級（JS dispatch／直接改 DOM）繞過的是 UI 層，判定一律 `⚠️`，不得填 `PASS`。
- **點擊必驗證且另起呼叫**：React 18 批次更新會讓同批次讀取拿到舊值；驗證要讀出具體狀態變化。
- **定位錨點入檔**：頁面文件的操作步驟帶 `id` 等穩定錨點，讓 Re-test 可重放而非重新探索。
...
│
├── merge-ui-test/          # 合併專案畫面測試的瀏覽器操作層（Browser pane 驅動、元件手冊、降級順序）
│   └── resources/          # 操作配方、元件手冊、產物格式、排障表
│
├── tools/                  # 同步工具
...
**文件版本**: v3.4
**變更記錄**（里程碑，最多 5 條）:
- v3.4 (2026-09-08): 新增第 16 個技能 `merge-ui-test`——合併專案畫面測試的瀏覽器操作層，與《合併專案畫面測試流程計畫書》分工為「操作 HOW」對「判定 WHAT」；元件手冊逐條標記 `[實測]`／`[通例]`，訂定五級降級順序並規定 JS 降級手段的 UI 判定不得填 `PASS`
