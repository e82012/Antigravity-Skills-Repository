...
**座標在任何狀態改變後即失效**，不只捲動：查詢結果筆數變了、彈窗開關、表單欄位增減，
版面都會位移。實測代理登入日誌搜索後結果由 30 筆變 2 筆，「清除搜索」按鈕從 y=476 移到
y=519，沿用舊座標的點擊直接落空且不報錯，差點被記成功能失效 `[實測]`。
**同一批次裡做過會改變版面的動作之後，後續目標要重新取座標。**
...
2. **用 `javascript_tool` 執行 `location.reload()` 或 `location.href=...` 換頁**，
   ...
   等於又用錯誤尺寸掛載一次。
3. 重載後確認搜索條件區存在，再開始測。
4. 反覆繪製失敗時，請使用者把 Claude 視窗帶到前景，依 §8 標 `BLOCKED`。
