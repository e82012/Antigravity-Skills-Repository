...
1. 先截一張圖強制繪製，確認 `innerWidth/innerHeight` 不是 0。
2. **用 `javascript_tool` 執行 `location.reload()` 或 `location.href=...` 換頁**，
   不要用 `navigate` 工具——實測 `navigate` 之後 viewport 會再度變成 `0×0`，
   等於又用錯誤尺寸掛載一次。
3. **重載後立刻連續截圖**，在掛載期間持續強制繪製。這是實測唯一穩定的做法：
...
4. **`resize_window` 救不回來**：實測元件只在掛載時量測一次，事後改變 viewport
   不會觸發重新量測。
5. 重載後確認搜索條件區存在，再開始測。
6. 反覆繪製失敗時，請使用者把 Claude 視窗帶到前景，依 §8 標 `BLOCKED`。
...
模擬開啟造成的不相等則無法換算補救——比例並非 `frameW / innerWidth`。
...
**文件版本**: v1.7
**變更記錄**（里程碑，最多 5 條）:
- v1.7 (2026-09-09): 補「重載後連續截圖」為讓元件正確掛載的唯一穩定做法，並註明 `resize_window` 無法補救；座標失效改列三個實測場景與「判 FAIL 前必先重取座標」紀律
- v1.6 (2026-09-09): 新增最優先的「畫面是否在繪製」檢查——視窗被遮蔽時 viewport 為 0×0，RWD 元件錯誤掛載會讓整個搜索條件區消失，且不自我修正；換頁改用 `location.href`／`location.reload()`，不用 `navigate`。補座標在任何版面變動後即失效
- v1.5 (2026-09-09): 明訂穩定組態為「關閉 viewport 模擬 ＋ pane 本身夠寬」；座標換算改為每次開工以輸入框探針驗證一次；補 pane 過窄會使整個搜索條件區不渲染
